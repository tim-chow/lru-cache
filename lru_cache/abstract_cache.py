...
class CacheStatus(object):
    ...
    def start(self, before_callback, after_callback):
        ...
        try:
            ...
            LOGGER.error("fail to run before callback",
                exc_info=True)
            with self._condition:
        ...
        try:
            ...
            LOGGER.error("fail to run after callback",
                exc_info=True)
        return True
...
class AbstractCache(object):
    ...
    def load_main(self):
        try:
            ...
            LOGGER.error("%s failed to " % self.name + 
                "load cache", exc_info=True)
    ...
    def stop(self, timeout=None):
        ...
        LOGGER.debug("begin to stop manager " + 
            "thread of %s" % self.name)
        if self._manager_thread != None:
            with self._condition:
                self._condition.notify()
            self._manager_thread.join(timeout)
            if self._manager_thread.isAlive():
                LOGGER.error("manager thread of %s " % self.name + 
                    "is still running")
            else:
                LOGGER.debug("manager thread of %s " % self.name + 
                    "is stopped")
                self._manager_thread = None
...
def test_abstract_cache():
    ...
    class Cache(AbstractCache):
        ...
        def load(self):
            for i in range(1, 11):
                LOGGER.debug("load loop #%02d, wait %.3fs" %
                    (i, i*0.1))
                yield i * 0.1
        ...
        def manage(self):
            ...
            while True:
                i = i + 1
                LOGGER.debug("manage loop #%03d, wait %.3fs" %
                    (i, (i%10)*0.1))
                yield (i % 10) * 0.1
